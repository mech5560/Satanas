 ...
 *  Last Modified: Time-stamp: <2014-06-04 11:17:51 mike_georgiou>
 *
 *
Intermediate_Velocity_Y_Corrector.cpp -- This function computes
the velocity_y_tilda implicitly over the y-direction. More
...
- velocity_y_old is introduced and is used on the computation of the
rhs.
...
void 
Intermediate_Velocity_Y_Corrector(double***velocity_y_tilda,
				  double*** residual_y,
  ...
  for (int k=0; k<ldz; k++){
	...
	Coefficient_Calculator(temperature, Reynolds,
	    ...
	    upper_diagonal[j+1] = coefficients[2];
            ...
            rhs[j+1] = ((rho[k][j][i]*velocity_y_old[k][j][i]+
